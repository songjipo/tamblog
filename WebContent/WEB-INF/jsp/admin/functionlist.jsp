...
<script type="text/javascript">
	...
	function doEdit() {
		...
		var row = $('#grid').datagrid('getSelected');  
       	...
       	parent.$.modalDialog({
			...
			width : 400,
			height : 400,
			href : '${ctx}/function/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function doDelete(){
		var ss = $('#grid').datagrid('getChecked');    
        if (ss.length == 0) {
            $.messager.alert('提示信息','请选择要禁用的权限','info');  
            return;
        }
        ...
        parent.$.messager.confirm('询问', '禁用权限，也将禁用该权限下所有权限，是否要禁用所选权限？', function(b) {
			if (b) {
				$.post('${ctx}/function/deleteBatch', {
					ids : ids
				}, function(result) {
					if (result.success) {
	...
	function doRestore(){
		var ss = $('#grid').datagrid('getChecked');    
        if (ss.length == 0) {
        	parent.$.messager.alert('提示信息', "请选择要恢复的权限", 'info');
            return;
        }
        ...
        parent.$.messager.confirm('询问', '恢复权限，也将恢复该权限下所有权限，是否要恢复所选权限？', function(b) {
			if (b) {
				$.post('${ctx}/function/restoreBatch', {
					ids : ids
				}, function(result) {
					if (result.success) {
						parent.$.messager.alert('提示信息', result.msg, 'info');
...
<body class="easyui-layout" style="visibility:hidden;" onkeydown="enterlogin();">
	<div region="center" border="false" title="权限列表">
    	<table id="grid"></table>
	</div>
